...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        x = self.self_attention(decoder_hidden_states, decoder_hidden_states)  # [batch_size, kv_seq_len, hidden]
        x = self.self_att_layer_norm(x + residual)  # [batch_size, kv_seq_len, hidden]

        residual = x
        x = self.cross_attention(decoder_hidden_states, encoder_hidden_states, key_padding_mask=key_padding_mask)
        x = self.cross_att_layer_norm(x + residual)  # [batch_size, kv_seq_len, hidden]

        residual = x
        x = self.fcn(x)
        x = self.dropout(x)
        x = self.fcn_layer_norm(x + residual)  # [batch_size, kv_seq_len, hidden]
...
class TransfomerEncoderDecoderModel(nn.Module):
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        output_embed = self.decoder_embeddings(decoder_input_ids)
        output_embed = self._add_positions(output_embed)

        decoder_layer_output = output_embed
        for decoder_layer in self.decoder_layers:
            decoder_layer_output = decoder_layer.forward(
                decoder_hidden_states=decoder_layer_output,
                encoder_hidden_states=encoder_hidden_states,
                key_padding_mask=key_padding_mask,
            )
        logits = self.out_proj(decoder_layer_output)
        # YOUR CODE ENDS HERE
        return logits
