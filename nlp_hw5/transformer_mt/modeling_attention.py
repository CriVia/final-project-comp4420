...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        if kv is None:
            k = self.k(q)  # [batch_size, query_seq_len, dim_k]
            v = self.v(q)  # [batch_size, query_seq_len, dim_v]

        else:
            k = self.k(kv)  # [batch_size, kv_seq_len, dim_k]
            v = self.v(kv)  # [batch_size, kv_seq_len, dim_v]

        q = self.q(q)   # [batch_size, query_seq_len, dim_v]
